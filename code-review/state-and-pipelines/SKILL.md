...
- **Computation interleaved with effects**: a function that calculates a total *and* writes the database *and* sends an email can only be tested end-to-end with mocks. Separate the pure core (`computeInvoice(items): Invoice`) from a thin effectful shell that calls it and performs the I/O — "functional core, imperative shell". The logic becomes trivially testable; the shell becomes trivially readable.
