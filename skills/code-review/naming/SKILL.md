...
- **Names that need a translator**: if a comment's main job is to define the name — "override" meaning the lifting of a billing freeze — the name failed; rename (`billingFreezeLiftRequested`) so the comment can shrink or vanish. Watch for **ceremony words** — `handle`, `process`, `resolve`, `manage`, `decide`, `-State`, `-Manager` — which name plumbing instead of mechanism: `manageAccountState` says neither that it takes a row lock nor what the lock protects; `lockAccountRow` says both.
