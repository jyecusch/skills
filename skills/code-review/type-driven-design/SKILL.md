...
Type-driven design does not mean annotating everything. Infer locals and private implementation details when the compiler already preserves the intended type. Write explicit types where they define a public contract, connect a runtime schema to its static representation, close a recursive type, or provide needed contextual typing. In Effect code, service contracts and meaningful success, error, and requirement channels are part of the API; do not discourage them, but do not restate a fully inferred `Effect` type on ordinary implementation locals.
